...
const initPanel = () => {
  ...
  window.__panelDebug = { state, grid, widgetManager: null, canvas };
  ...
  window.__panelDebug.widgetManager = widgetManager;
  ...
  const buildToolbox = () => {
    ...
    select.addEventListener('change', (event) => {
      ...
      console.debug('[panel] toolbox selected tool', state.pendingTool);
  ...
  const handleCanvasClick = (event) => {
    ...
    console.debug('[panel] click currentTarget', event.currentTarget);
    console.debug('[panel] click mode', state.mode, 'pending tool', state.pendingTool);
    if (state.mode === 'run') return;
    ...
    console.debug('[panel] click coords', coords);
    if (!coords) return;
    console.debug('[panel] addWidget', state.pendingTool, coords);
    const widget = widgetManager.addWidget({ type: state.pendingTool, pos: coords });
    console.debug('[panel] widgets size', widgetManager.widgets.size, 'canvas children', canvas.children.length);
    if (widget?.id) {
